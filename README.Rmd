...
ecgate also seems to have an issue with the {_systemfonts_} package that helps to provide a wide choice of fonts for plots. This can be alleviated by installing an older version of {_systemfonts_} before attempting to install {_harp_} using `remotes::install_version`:
```{r eval=FALSE}
